# The code here is used after rescue_Factiva_csv.py can no longer find more urls for the left articles
...
# Different from rescue_Factiva_csv.py, the search query used here is LP text, not TD text
# The search query used here is not saved in Factiva_csv.csv, even if the url can be found
import re
import csv
...
class MLStripper(HTMLParser):
    ...
    def get_data(self):
        ...
        if '@globeandmail.com' in result:
            return result.split('@globeandmail.com')[1]
        return result
...
def process_url(search_query):
    ...
    try:
        ...
        if iter(sys.exc_info()[1]):
            exception_str = ','.join(['exception', str(sys.exc_info()[1])])
            print(exception_str)
            return(exception_str, exception_str)
        else:
...
def main():
    ...
    with open(uncaptured_file) as csvfile:
        ...
        for r in f:
            ...
            print(file_idx)
            if 'exception' not in r['URL'] and r['URL'] != '' and r['Article_ID'] == '':
                uncaptured_dct[file_idx] = r
                continue
            if outoflimitation_count > exception_limit:    # this happened when you used all Google CSE daily limit
                uncaptured_dct[file_idx] = r
                continue

            search_query_3 = strip_tags(r['LP'])
            r['Search_Query_3'] = search_query_3
            rescue_url, rescure_article_id = process_url(search_query_3)
            r['URL'] = rescue_url
            r['Article_ID'] = rescure_article_id
            if 'article' not in rescue_url:
                if '403' in rescure_article_id:
                    outoflimitation_count += 1
                uncaptured_dct[file_idx] = r
